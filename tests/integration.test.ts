...
describe('agledger_api (live)', () => {
  ...
  it('POST /v1/records creates a record', async () => {
    ...
    expect(content.type).toBe('notarize-generic-v1');
  });
  ...
  it('returns structured error with schema help for bad criteria', async () => {
    ...
    expect(content.recordType).toBeDefined();
  });
});
...
describe('full record lifecycle (live)', () => {
  ...
  it('step 3: submit completion', async () => {
    if (skipIfNoApi() || !recordId) return;
    ...
    const result = await client.callTool({
      ...
      arguments: {
        method: 'POST',
        path: `/v1/records/${recordId}/completions`,
        params: {
          evidence: {
            summary: 'Lifecycle test record delivered',
            evidenceUrl: 'https://example.com/mcp-lifecycle-001',
          },
        },
      },
    });
  ...
  it('step 4: check record reached terminal state', async () => {
    ...
    expect(['FULFILLED', 'FAILED', 'PROCESSING']).toContain(status);
  });

  it('step 5: list completions for record', async () => {
    if (skipIfNoApi() || !recordId) return;

    const result = await client.callTool({
      name: 'agledger_api',
      arguments: { method: 'GET', path: `/v1/records/${recordId}/completions` },
    });
